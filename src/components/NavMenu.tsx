...
const NavMenu: React.FC<NavMenuProps> = ({ onDeckSelect }) => {
  ...
  return (
    <div className={`fixed ${isVertical ? 'left-4 top-1/2 transform -translate-y-1/2' : 'bottom-4 left-1/2 transform -translate-x-1/2'} z-50`}>
      <div className={`bg-white bg-opacity-20 backdrop-blur-lg rounded-full p-2 shadow-lg flex ${isVertical ? 'flex-col items-center space-y-2' : 'items-center space-x-2'}`}>
        {isVertical && (
          <NavIcon
        ...
        {isVertical ? <div className="w-8 h-px bg-gray-300 my-2"></div> : <div className="h-8 w-px bg-gray-300 mx-2"></div>}
...
const NavIcon: React.FC<NavIconProps> = ({ href, onClick, icon: Icon, label, index, hoveredIndex, setHoveredIndex, isVertical }) => {
  ...
  const content = (
    ...
    >
      ...
      <span className={`text-xs mt-1 absolute ${isVertical ? 'left-full ml-2 top-1/2 -translate-y-1/2' : '-bottom-6 left-1/2 -translate-x-1/2'} whitespace-nowrap bg-gray-800 text-white px-2 py-1 rounded transition-all duration-200 ${isHovered ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
        {label}
      </span>
    </div>
  );
